...
一般不经常进行两个人之间直接推版本库，一般都是有一个中央服务器。
...
![查看本地分支和远程分支的关联关系](./查看本地分支和远程分支的关联关系.png)
...
1. 在test01这个本地仓库进行一次提交并推送到远程仓库
   ![在test01这个本地仓库进行一次提交并推送到远程仓库](./在test01这个本地仓库进行一次提交并推送到远程仓库.png)
2. 在另一个仓库将远程提交的代码拉取到本地仓库
   ![将仓库里远程提交的代码拉取到本地仓库](./将仓库里远程提交的代码拉取到本地仓库.png)
...
![在Idea中配置Git](./在Idea中配置Git.png)

点击Test按钮,现在执行成功，配置完成

![在Idea中成功配置Git](./在Idea中成功配置Git.png)
...
![IDEA-创建项目远程仓库](./IDEA-创建项目远程仓库.png)

#### 5.2.2、初始化本地仓库

![IDEA-初始化本地仓库](./IDEA-初始化本地仓库.png)

#### 5.2.3、设置远程仓库

![IDEA-设置远程仓库](./IDEA-设置远程仓库.png)

#### 5.2.4、提交到本地仓库

![IDEA-提交到本地仓库](./IDEA-提交到本地仓库.png)

#### 5.2.6、推送到远程仓库

![IDEA-推送到远程仓库](./IDEA-推送到远程仓库.png)

#### 5.2.7、克隆远程仓库到本地

![IDEA-克隆远程仓库到本地](./IDEA-克隆远程仓库到本地.png)

#### 5.2.8、创建分支

- 最常规的方式
  ![IDEA-创建分支（常规）](./IDEA-创建分支（常规）.png)
- 最强大的的方式
  ![IDEA-创建分支（强大）](./IDEA-创建分支（强大）.png)

#### 5.2.9、切换分支及其他分支相关操作

![IDEA-切换分支及其他分支相关操作](./IDEA-切换分支及其他分支相关操作.png)

#### 5.2.11、解决冲突

1. 执行merge或pull操作时，可能发生冲突
   ![IDEA-解决冲突](./IDEA-解决冲突.png)
2. 冲突解决后加入暂存区
   略
3. 提交到本地仓库
   略
...
1. 第一张图上的快捷入口可以基本满足开发的需求。
   ![IDEA常用GIT操作入口](./IDEA常用GIT操作入口1.png)
2. 第二张图是更多在IDEA操作git的入口。
   ![IDEA常用GIT操作入口](./IDEA常用GIT操作入口2.png)
...
![windows下查看隐藏文件](./windows下查看隐藏文件.png)
...
![windows下创建.ignore、.bashrc文件](./windows下创建.ignore、.bashrc文件.png)

## 附：IDEA集成GitBash作为Terminal

![IDEA集成GitBash作为Terminal](./IDEA集成GitBash作为Terminal.png)
