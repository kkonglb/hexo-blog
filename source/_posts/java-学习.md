...
categories:
  - 技能积累
date: 2025-03-17 21:37:52
---

 [java 教程](https://www.runoob.com/java/java-intro.html)

# Java教程

[java在线工具](https://www.runoob.com/try/runcode.php?filename=HelloWorld&type=java)
...
![img](java-学习/662E827A-FA32-4464-B0BD-40087F429E98.jpg)
...
[菜鸟教程](https://www.runoob.com/java/java-object-classes.html)
...
```java
//不错的学习例子
// 虽然加了public修饰符，但是这个也是一个普通的类，具有类的共性
public class Puppy{
   public Puppy(String name){
      //这个构造器仅有一个参数：name
      System.out.println("小狗的名字是 : " + name ); 
   }
    //感觉主函数main函数里面是执行具体语句
   public static void main(String[] args){
      // 下面的语句将创建一个Puppy对象
...
编译运行多个文件，其中EmployeeTest中有main函数
...
奇怪，为什么我在vscode能直接编译运行EmployeeTest这个文件而不需要Employee.class
...
数据范围不用记, 可以看下下面的代码   都是第一个字母大写,然后方法是全部大写.
...
- **局部变量（Local Variables）** 这个简单理解就是代码块里面的变量
- **实例变量（Instance Variables）** 这个是类里面的变量，差不多相当于全局变量
- **静态变量或类变量（Class Variables）** 这个就是实例变量加了个 static，是类的一个固有属性，可以通过类直接访问，所有对象共用这一个变量
- **参数变量（Parameters）** 顾名思义就是方法中的参数
...
default ：默认访问修饰符的访问级别是包级别（package-level），即只能被同一包中的其他类访问。
...
Number和Math都在java.lang包中，java.lang已经默认导入了
...
```java
String str = "Runoob";  //公共池
String str2=new String("Runoob"); //堆
```
...
```java
printArray(new int[]{3, 1, 2, 6, 4, 2});//这个可以学
```
...
![img](java-学习/D53C92B3-9643-4871-8A72-33D491299653.jpg)
